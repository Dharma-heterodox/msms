...
public class SwaggerConfig {
	...
	public Docket schoolApi() {
		...
		parameters.add(new ParameterBuilder().name("X-TenantID").description("Tenant ID")
				.modelRef(new ModelRef("string")).parameterType("header").required(false).build());
		return new Docket(DocumentationType.SWAGGER_2).select()
				.apis(RequestHandlerSelectors.basePackage("com.sms.core.controller")).paths(PathSelectors.any()).build()
				.apiInfo(apiInfo()).globalOperationParameters(parameters);
	}

	@SuppressWarnings("deprecation")
	public ApiInfo apiInfo() {
		ApiInfo apiInfo = new ApiInfo("powerplay", "School management system", "1.0", "powerplay", "powerplay",
				"powerplay", "powerplay");
		return apiInfo;
	}
}
